...
def calculate_amino_acid_masses(peptide_sequence, scale, resin_loading, mode, amino_acid_weights):
    ...
    amino_acid_comments = {aa: f"Fmoc-{aa}" for aa in amino_acid_weights}
...
def streamlit_main():
    ...
    # Input fields for new amino acid
    ...
    return sequence, scale, resin_loading, mode, new_aa, new_aa_weight

def display_amino_acid_weights(amino_acid_weights):
    df = pd.DataFrame({
        "Amino Acid": amino_acid_weights.keys(),
        "Molecular weight": amino_acid_weights.values()
    })
...
if __name__ == "__main__":
    ...
    sequence, scale, resin_loading, mode, new_aa, new_aa_weight = streamlit_main()

    if new_aa and new_aa_weight:
        amino_acid_weights[new_aa] = new_aa_weight
    
    df = calculate_amino_acid_masses(sequence, scale, resin_loading, mode, amino_acid_weights)
